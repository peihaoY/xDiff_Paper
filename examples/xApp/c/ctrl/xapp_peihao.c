...
void gen_rrm_policy_ratio_list(seq_ran_param_t* RRM_Policy_Ratio_List)
{
  ...
  int numbers[2];
  FILE *file = fopen("rlflexric/dudata/slice_ctrl.bin", "rb");
  if (file == NULL) {
    perror("Unable to open slice_ctrl.bin file \n");
    //exit(0);
  }
  
  size_t read = fread(numbers, sizeof(int), 2, file);
  if (read != 2) {
    perror("Error reading from file: read numbers != 2");
    fclose(file);
    exit(0);
  }
  ...
  int dedicated_ratio_prb[] = {numbers[0], numbers[1]};
  int Max_PRB_Ratio[] = {80, 80};
  int Min_PRB_Ratio[] = {20, 20};
